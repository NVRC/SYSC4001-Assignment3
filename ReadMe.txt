...
the richest man in world has 42 billion dollars so it seemed reasonable.

dbEditor:
The is part the atm program is a separte process from the rest of the program it runs concurrently with the ATM
to update the DB's text file. This update could either be adding a new account to the database or modifying an 
...
the dbEditor does not actually change the DB. This is performed by the Server the dbEditor sends the account information 
